...
func heapifyMinHeap(_ arr: inout [Int], n: Int, i: Int) {
    var largest = i
    let left = 2 * i + 1
    let right = 2 * i + 2
    ...
    if largest != i {
        arr.swapAt(i, largest)
        heapifyMinHeap(&arr, n: n, i: largest)
    }
}


func heapSortMinheap(_ arr: inout [Int]) {
    let n = arr.count

    for i in stride(from: (arr.count - 1)/2, through: 0, by: -1) {
        heapifyMinHeap(&arr, n: n, i: i)
    }

    for i in stride(from: arr.count - 1 , through: 0, by: -1) {
        arr.swapAt(0, i)
        heapifyMinHeap(&arr, n: i, i: 0)
    }
}
